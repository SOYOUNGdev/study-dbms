...
values('제임스', 55, '서울 송파구', '01011112222', '남자');

insert into tbl_parent
(name, age, address, phone)
values('크리스티나', 47, '경기도 남양주', '01033332222');
...
/*select 8, name as 또치, age + 1, gender, address, phone from tbl_parent;
select 9, name as 둘리, age + 1, gender, address, phone from tbl_parent;*/
...
/*
    ...
    'A%' : A로 시작하는 모든 값(ASDS, ASFCXD, ...)
    '%A%' : A가 포함된 모든값(Java, banana, ...)
*/
...
/*집계함수는 where절에서 사용할 수 없다. select절에서만 사용할 수 있다.*/
select name, round(avg(price), 0) average from tbl_order
...
/*상품의 평균 가격이 1000원 이하인 상품의 개별 조회*/
...
/*
    ...
    1. from
    2. join
    3. on       (조인 전 테이블에서 끊을 수 있는 거 미리 다 끊기)
    4. where    (테이블 조인해야만 끊을 수 있는 정보)
    5. group by
    6. having
    7. select
    8. order by
...
select c.* from tbl_child c join tbl_apply a
                            ...
                            on f.title like '%매미%'
                            and f.id = a.field_trip_id;


/*체험학습을 2개 이상 신청한 아이의 정보와 부모의 정보 모두 조회*/
select * from tbl_apply;
select * from tbl_parent p join(select c.* from tbl_child c join(
                                                                select child_id from tbl_apply a
                                                                group by a.child_id
                                                                having count(a.field_trip_id) >= 2) ac
                                on c.id = ac.child_id) cac
on p.id = cac.parent_id;

/*참가자(지원자) 수가 가장 적은 체험학습의 제목과 내용 조회*/
/*
    z in a,b,c: in절 뒤 컬럼의 개수가 2개 이상일 경우 사용.(in -> z=a or z=b or z=c)
    z = a : = 뒤 컬럼의 개수가 1개일 때 사용.(z=a)
*/
select f.title, f.content from tbl_field_trip f join tbl_apply a
                                                on f.id = a.field_trip_id
                                                group by f.id
                                                having count(a.child_id) = (select min(cn) from (select count(child_id) cn
                                                                     from tbl_apply
                                                                     group by field_trip_id) cn);

/*전체평균 참가자(지원자) 수보다 적은 체험학습의 제목과 내용 조회*/
/*
    apply 테이블에는 지원자 수가 0인 체험학습의 정보는 없다.
    따라서 외부 조인을 사용하여 field_trip 테이블의 정보를 전부 가져와야한다.
*/
/*total cost : */
select f.title, f.content from tbl_field_trip f left outer join tbl_apply a
                                                on f.id = a.field_trip_id
                                                group by f.id
                                                having count(a.child_id) < (select floor(avg(cn)) from (select count(child_id) cn from tbl_apply
                                                group by field_trip_id) cn);
